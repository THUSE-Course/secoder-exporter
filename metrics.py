...
def render_metrics(conn: Connection) -> str:
    ...
    lines.append(
        "# HELP secgit_commits_count Stored first-parent commits per author"
    )
    lines.append("# TYPE secgit_commits_count gauge")
    lines.append(
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        ...
        for row in cur.fetchall():
            ...
            lines.append(f"secgit_commits_count{{{labels}}} {row['c']}")
